...
pub fn anniversary_get_value(str: &str, kind: &Option<ValueKind>) -> Result<ValueData, VcardError> {
...
pub fn anniversary_allowed_parameter(parameter_type: &ParameterType) -> Result<(), VcardError> {
    match parameter_type {
        ParameterType::AltId => Ok(()),
        ParameterType::Any => Ok(()),
        ParameterType::CalScale => Ok(()),
        ParameterType::Value => Ok(()),
        _ => Err(VcardError::ParameterTypeNotAllowed(parameter_type.to_string(), String::from(PROPERTY_TYPE_ANNIVERSARY))),
    }
}
