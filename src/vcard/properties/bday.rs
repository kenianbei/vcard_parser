...
mod tests {
    ...
    pub fn bday_valid() {
        ...
        assert!(matches!(result, Ok(ValueData::Date((2000, 1, 1)))));
        let result = bday_get_value("Sat, 01 Jan 2000 00:00:00 GMT", &Some(ValueKind::Date));
        assert!(matches!(result, Ok(ValueData::Date((2000, 1, 1)))));
        let result = bday_get_value("2000-01-01T00:00:00.000000000-00:00", &Some(ValueKind::Date));
        assert!(matches!(result, Ok(ValueData::Date((2000, 1, 1)))));
    }
}
