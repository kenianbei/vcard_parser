...
use crate::vcard::property::types::PROPERTY_TYPE_BIRTHPLACE;
use crate::vcard::values::data::ValueData;
use crate::vcard::values::kind::ValueKind;
use crate::VcardError;

pub fn birthplace_get_value(str: &str, kind: &Option<ValueKind>) -> Result<ValueData, VcardError> {
...
pub fn birthplace_allowed_parameter(parameter_type: &ParameterType) -> Result<(), VcardError> {
    match parameter_type {
        ParameterType::AltId => Ok(()),
        ParameterType::Any => Ok(()),
        ParameterType::Language => Ok(()),
        ParameterType::Value => Ok(()),
        _ => Err(VcardError::ParameterTypeNotAllowed(parameter_type.to_string(), String::from(PROPERTY_TYPE_BIRTHPLACE))),
    }
}
