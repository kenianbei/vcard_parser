...
use crate::vcard::property::types::PROPERTY_TYPE_IMPP;
use crate::vcard::values::data::ValueData;
use crate::vcard::values::kind::ValueKind;
use crate::VcardError;

pub fn impp_get_value(str: &str, kind: &Option<ValueKind>) -> Result<ValueData, VcardError> {
    ...
    match Url::parse(str) {
        ...
        Err(_) => Err(VcardError::PropertyValueInvalid(PROPERTY_TYPE_IMPP.to_string())),
    }
}

pub fn impp_allowed_parameter(parameter_type: &ParameterType) -> Result<(), VcardError> {
    match parameter_type {
        ...
        _ => Err(VcardError::ParameterTypeNotAllowed(parameter_type.to_string(), String::from(PROPERTY_TYPE_IMPP))),
    }
}
...
mod tests {
    ...
    #[test]
    pub fn impp_valid() {
        let result = impp_get_value("", &Some(ValueKind::Text));
        assert!(matches!(result, Err(VcardError::ValueKindNotAllowed(_, _))));
        let result = impp_get_value("", &Some(ValueKind::Uri));
        assert!(matches!(result, Err(VcardError::PropertyValueInvalid(_))));
        let result = impp_get_value("xmpp:alice@example.com", &Some(ValueKind::Uri));
        assert!(matches!(result, Ok(ValueData::Uri(_))));
        let result = impp_get_value("xmpp:alice@example.com", &None);
        assert!(matches!(result, Ok(ValueData::Uri(_))));
    }
}
