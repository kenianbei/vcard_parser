...
use crate::vcard::property::types::PROPERTY_TYPE_SOURCE;
use crate::vcard::values::data::ValueData;
use crate::vcard::values::kind::ValueKind;
use crate::VcardError;

pub fn source_get_value(str: &str, kind: &Option<ValueKind>) -> Result<ValueData, VcardError> {
    ...
    match Url::parse(str) {
        ...
        Err(_) => Err(VcardError::PropertyValueInvalid(PROPERTY_TYPE_SOURCE.to_string())),
    }
}

pub fn source_allowed_parameter(parameter_type: &ParameterType) -> Result<(), VcardError> {
    match parameter_type {
        ...
        _ => Err(VcardError::ParameterTypeNotAllowed(parameter_type.to_string(), String::from(PROPERTY_TYPE_SOURCE))),
    }
}
...
mod tests {
    ...
    #[test]
    pub fn source_valid() {
        let result = source_get_value("", &Some(ValueKind::Text));
        assert!(matches!(result, Err(VcardError::ValueKindNotAllowed(_, _))));
        let result = source_get_value("", &Some(ValueKind::Uri));
        assert!(matches!(result, Err(VcardError::PropertyValueInvalid(_))));
        let result = source_get_value("ldap://ldap.example.com/cn=Babs%20Jensen,%20o=Babsco,%20c=US", &Some(ValueKind::Uri));
